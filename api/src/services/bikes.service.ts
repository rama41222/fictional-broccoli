...
const bikesTransformer = async (data: TransformType, weather: string): Promise<void | Record<string, any>> => {
    try {
        const { features } = await fetchBikes(process.env.BIKE_SHARING_API as string);
